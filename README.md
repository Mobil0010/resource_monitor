...
- 팝업 모니터, 위치, 크기, 배경 투명도와 미니 그래프 설정
- 팝업 뒤에 있는 게임과 앱을 그대로 조작할 수 있는 클릭 통과
- 메인 앱과 데이터를 공유하며 별도 프로세스를 만들지 않는 보조 팝업
- 로컬 Steam 라이브러리 자동 검색 및 직접 경로 지정
- 전체 또는 선택한 Steam 게임의 실행 상태 감지
- Steam 게임 공통 설정과 게임별 전용 팝업 설정
- Windows Steam 게임 팝업의 FPS와 프레임 시간 표시
- 시스템 부팅 시 자동 실행
...
팝업은 다른 창보다 항상 위에 표시되며 배경 투명도를 0%부터 100%까지 조절할 수 있습니다. 0%에서는 배경이 완전히 사라지고 글씨와 그래프만 남으며, 글씨와 그래프의 선명도는 배경 투명도와 관계없이 유지됩니다. 팝업은 마우스 입력을 가로채지 않으므로 팝업 뒤에 있는 게임이나 앱을 그대로 클릭할 수 있습니다.

선택한 정보의 최근 사용량 그래프도 한 줄에 두 개씩 표시할 수 있습니다. 디스크 사용률과 프로세스 수는 값만 표시하고 그래프는 만들지 않습니다. 지원되는 센서가 있으면 CPU, GPU 또는 디스크 값 옆에 온도도 표시합니다. 여러 모니터를 사용하는 경우 표시할 모니터를 선택할 수 있으며, 각 모니터의 해상도와 작업 영역을 기준으로 팝업 위치를 계산합니다. 메인 창의 일반 닫기 버튼을 눌러도 앱과 팝업은 백그라운드에서 유지되며, 사이드바의 프로그램 종료 버튼으로 완전히 종료할 수 있습니다.
...
사이드바의 `게임 연동`에서 사용자가 원할 때만 로컬 Steam 라이브러리를 연결할 수 있습니다. Steam 계정 로그인, 비밀번호 또는 Web API 키는 필요하지 않으며, 로컬에 설치된 라이브러리 manifest와 아이콘 캐시를 읽어 게임 목록을 구성합니다.

- 기본 Steam 설치 위치와 추가 라이브러리 폴더 자동 검색
- 원하는 Steam 라이브러리 경로 직접 지정
- 설치된 전체 게임 또는 직접 선택한 게임만 실행 감지
- 게임, DLC, 데모와 일반 프로그램·도구를 별도 그룹으로 분류
- Wallpaper Engine, Lossless Scaling, Steamworks Common Redistributables 및 호환성 도구는 `기타`로 분류
- `기타` 항목을 팝업 실행 대상에 포함할지 별도로 선택
- 게임 실행 중 일반 팝업을 숨기고 게임용 팝업 설정 적용
- 게임 종료 후 기존 일반 팝업 상태 자동 복원
- 모든 게임이 공유하는 설정 또는 게임마다 다른 전용 설정 사용
- 게임별 감지용 실행 파일 추가 지원

게임 팝업은 처음에 일반 팝업의 표시 항목, 모니터와 화면 위치, 크기, 배경 투명도 및 새로고침 간격을 그대로 사용합니다. 원하는 경우 게임 공통 설정을 일반 팝업과 분리할 수 있고, 특정 게임에만 별도의 설정을 적용할 수도 있습니다.

Windows에서는 게임 팝업에 FPS와 프레임 시간(ms)을 추가할 수 있습니다. 이 값은 게임 프로세스가 실행 중이고 측정 항목을 활성화했을 때 PresentMon으로 수집합니다. macOS에서는 현재 FPS와 프레임 시간 측정을 지원하지 않습니다.

## Windows 추가 구성요소

### PawnIO

PawnIO는 Windows가 일반적인 시스템 API로 제공하지 않는 CPU와 디스크 등의 하드웨어 온도 센서에 접근할 때 사용합니다. Resource Monitor를 처음 실행하면 PawnIO 설치 창과 Windows 관리자 권한 요청이 나타날 수 있습니다. 설치하지 않아도 CPU·메모리·네트워크 같은 기본 모니터링은 사용할 수 있지만, 일부 온도는 표시되지 않을 수 있습니다.

### PresentMon

PresentMon은 Windows에서 Steam 게임의 FPS와 프레임 시간을 측정할 때 사용합니다. 정식 배포되는 Windows 설치 마법사와 Portable ZIP에는 검증된 PresentMon 실행 파일과 라이선스가 `tools` 폴더에 함께 포함되므로 사용자가 별도로 설치할 필요가 없습니다. FPS 또는 프레임 시간 표시를 활성화한 게임이 실행 중일 때만 측정에 사용합니다.

`cargo run`으로 직접 개발 실행한 경우에는 배포 패키지의 `tools/PresentMon.exe`가 없으므로 FPS와 프레임 시간이 표시되지 않을 수 있습니다. 이 기능까지 로컬에서 시험하려면 공식 PresentMon 실행 파일을 빌드된 Resource Monitor 실행 파일과 같은 폴더 또는 그 아래 `tools` 폴더에 배치해야 합니다.
...
4. `게임 연동`에서 필요할 때만 로컬 Steam 라이브러리를 연결하고 팝업 실행 대상과 게임별 설정을 선택해 주세요.
5. `Settings`에서 언어, 일반 팝업 항목, 미니 그래프, 모니터, 위치, 크기, 배경 투명도와 자동 실행 여부를 설정해 주세요.
6. 오른쪽 위 버튼으로 라이트/다크 테마를 전환할 수 있습니다.
7. 메인 창과 팝업을 모두 닫으려면 사이드바의 프로그램 종료 버튼을 눌러 주세요.
...
- [PresentMon](https://github.com/GameTechDev/PresentMon) — Windows 게임 FPS 및 프레임 시간 측정
...
- 온도 표시 여부는 운영체제, 하드웨어 센서와 드라이버 지원 상태에 따라 달라집니다.
- 메모리 온도는 일반적인 운영체제 API에서 제공되지 않는 경우가 많아 대부분의 환경에서 표시되지 않을 수 있습니다.
- Steam 게임 분류는 로컬 manifest 정보와 Steam 메타데이터를 우선 사용하며, 일부 항목은 이름을 이용한 보조 분류가 적용됩니다.
