...
1. Cargo.toml의 package.version을 배포할 버전으로 설정해 주세요.
2. Cargo.lock도 필요한 경우 갱신하고 변경 사항을 커밋하여 푸시해 주세요.
3. 같은 버전의 새 태그를 만들어 푸시해 주세요.

예를 들어 Cargo.toml의 버전이 0.1.0인 경우:
...
이미 존재하는 태그는 덮어쓰지 말고 새로운 버전을 사용해 주세요. 현재 워크플로는 v숫자.숫자.숫자 형식의 정식 버전 태그만 허용합니다.
...
두 운영체제의 빌드가 모두 성공하면 초안 릴리스에 파일을 업로드한 뒤 공개합니다. 실패한 실행은 재실행할 수 있지만, 이미 공개된 릴리스의 파일은 자동으로 덮어쓰지 않습니다. 수동 실행 시에는 브랜치가 아니라 기존 버전 태그를 선택해야 합니다.
